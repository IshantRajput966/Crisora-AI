...
const SkeletonCard = () => (
  <div className="bg-slate-800 rounded-xl border border-slate-700 p-4 animate-pulse">
    ...
    <div className="h-4 bg-slate-700 rounded w-1/2 mx-auto mt-6"></div>
  </div>
);
  ...
  return (
    <div className="space-y-6 pb-24 font-sans relative">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-slate-100">Citizen Dashboard</h1>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* District Risk Card */}
        {isLoading ? <SkeletonCard /> : (
          <div className="bg-slate-800 rounded-xl border border-slate-700 p-4 shadow-xl relative overflow-hidden">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-lg font-semibold text-slate-200 flex items-center gap-2">
                <ShieldCheck className="text-emerald-500" size={20} /> My District Risk
              </h2>
            </div>
        ...
        {isLoading ? <SkeletonCard /> : (
          <div className="bg-slate-800 rounded-xl border border-slate-700 p-4 shadow-xl">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-lg font-semibold text-slate-200 flex items-center gap-2">
                <Bell className="text-amber-500" size={20} /> Active Alerts
              </h2>
              <span className="px-3 py-1 bg-red-900/50 text-red-400 border border-red-800 rounded-full text-xs font-bold">
            ...
            {recentAlerts.length > 0 ? (
              <ul className="space-y-4">
                {recentAlerts.map(alert => (
                  <li key={alert.id} className="flex items-start gap-3 p-3 bg-slate-900/50 rounded-lg border border-slate-700/50">
                    <AlertTriangle className="text-orange-500 flex-shrink-0 mt-0.5" size={16} />
                    <div>
                      <p className="text-sm text-slate-200 font-medium">{alert.type || alert.message || 'Emergency Request'}</p>
                      <p className="text-xs text-slate-400 mt-1">{alert.location || 'Unknown Location'}</p>
      ...
      {/* Map Section */}
      <div className="bg-slate-800 rounded-xl border border-slate-700 p-4 shadow-xl">
        <h2 className="text-lg font-semibold text-slate-200 mb-4 px-2">Live District Map</h2>
        <div className="h-[350px] w-full rounded-lg overflow-hidden border border-slate-700">
          <DisasterMap 
      ...
      <div className="bg-slate-800 rounded-xl border border-slate-700 p-4 shadow-xl">
        <h2 className="text-lg font-semibold text-slate-200 mb-6 flex items-center gap-2">
          <HeartHandshake className="text-blue-500" size={20} /> Community Help
        </h2>
        {districtAlerts.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {districtAlerts.map(alert => (
              <div key={alert.id} className="p-4 bg-slate-900/80 rounded-lg border border-slate-700 flex justify-between items-center">
                <div>
                  <h3 className="text-sm font-medium text-slate-200">{alert.message || 'Assistance required'}</h3>
                  <p className="text-xs text-slate-400 mt-1">{alert.location || 'Location tracking active'}</p>
                </div>
                <button 
                  onClick={() => handleHelpClick(alert.id)}
                  className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-blue-400 text-sm font-medium rounded-lg transition-colors border border-slate-600"
                >
                  I can help
                </button>
              </div>
          ...
          <p className="text-sm text-slate-400 text-center py-8 bg-slate-900/50 rounded-lg border border-slate-700/50">
      ...
      */}
      <div className="fixed bottom-0 left-0 right-0 lg:left-60 bg-slate-800/95 backdrop-blur-md border-t border-slate-700 p-4 flex justify-between items-center z-[1001] px-6">
        <SOSButton />
        <Link 
          to="/citizen/ai" 
          className="flex items-center gap-2 px-5 py-2.5 bg-indigo-600/20 hover:bg-indigo-600/30 text-indigo-400 border border-indigo-500/30 rounded-full font-medium transition-colors"
        >
          <Bot size={18} />
