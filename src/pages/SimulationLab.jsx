    ...
    <div className="space-y-8 font-sans pb-24 w-full max-w-7xl mx-auto px-2">
      ...
      <div>
        ...
        <p className="text-slate-400 mt-2 text-lg">
          Replay past Indian disasters and see how <span className="text-blue-400 font-semibold">Crisora AI</span> would have predicted and responded.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {HISTORICAL_EVENTS.map(ev => (
          <div 
            key={ev.id} 
            className={`bg-slate-800 rounded-2xl border ${selectedEvent?.id === ev.id ? 'border-amber-500 shadow-[0_0_15px_rgba(245,158,11,0.2)]' : 'border-slate-700'} shadow-xl p-6 transition-all hover:-translate-y-1`}
          >
            <div className="flex justify-between items-start mb-4">
              <div className="p-3 bg-slate-900 rounded-xl shadow-inner border border-slate-700">
                {getEventIcon(ev.type)}
              </div>
              <div className="text-right">
                <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">{ev.year}</span>
                <div className="flex gap-1 mt-1 justify-end" title={`Severity: ${ev.severity}/5`}>
                  {[1, 2, 3, 4, 5].map(dot => (
                    <div key={dot} className={`w-2 h-2 rounded-full ${dot <= ev.severity ? 'bg-red-500' : 'bg-slate-600'}`} />
                  ))}
                </div>
              </div>
            </div>
            <h3 className="text-xl font-bold text-slate-100 mb-1">{ev.name}</h3>
            ...
            <button 
              ...
              className="w-full py-3 bg-amber-600 hover:bg-amber-500 text-white font-bold uppercase tracking-wider rounded-xl transition-colors shadow-lg shadow-amber-600/20"
            >
              Simulate Scenario
            </button>
          </div>
      ...
      {/* Simulation Panel */}
      {selectedEvent && (
        <div id="sim-panel" className="bg-slate-800 rounded-3xl border border-slate-700 shadow-2xl p-6 md:p-8 animate-in slide-in-from-bottom-8 fade-in duration-500 mt-12">
          <div className="flex flex-col lg:flex-row gap-8">
            
            {/* Left Controls & Insights */}
            <div className="w-full lg:w-1/3 flex flex-col gap-6">
              <h2 className="text-2xl font-bold text-slate-100 border-b border-slate-700 pb-4">
                {selectedEvent.name} <span className="text-slate-500 text-lg">({selectedEvent.year})</span>
              </h2>
              ...
              <div className="bg-slate-900/50 p-6 rounded-2xl border border-slate-700 shadow-inner">
                <div className="flex justify-between items-center mb-6">
                  <span className="text-sm font-bold text-slate-300 uppercase tracking-widest">
                    T-Minus <span className="text-amber-400 text-lg mx-1">{hoursRemaining}</span> Hours
                  </span>
                  <button 
                    onClick={() => setIsPlaying(!isPlaying)}
                    className="p-3 bg-blue-600 hover:bg-blue-500 text-white rounded-full shadow-lg shadow-blue-600/30 transition-colors"
                  >
                ...
                />
                <div className="flex justify-between text-xs text-slate-500 font-bold mt-3">
                  <span>-72H</span>
                  <span>-36H</span>
                  <span className={timeline === 72 ? 'text-red-500 animate-pulse' : ''}>0H (STRIKE)</span>
                </div>
              </div>
              ...
              <div className="bg-blue-900/10 border border-blue-500/30 p-5 rounded-2xl transition-colors duration-500">
              ...
              {/* Outcome Card at T-0 */}
              {timeline === 72 && (
                <div className="bg-red-900/20 border border-red-500/50 p-5 rounded-2xl animate-in zoom-in fade-in duration-500">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between border-b border-red-500/20 pb-3">
                      <span className="text-slate-400 text-sm">Casualties</span>
                      <span className="font-bold text-slate-100 text-lg">{selectedEvent.outcome.casualties}</span>
                    </div>
                    <div className="flex justify-between pt-1">
                      <span className="text-slate-400 text-sm">Est. Damage</span>
                      <span className="font-bold text-slate-100 text-lg">{selectedEvent.outcome.damage}</span>
                    </div>
                  </div>
                </div>
              )}
            ...
            {/* Right Map */}
            <div className="w-full lg:w-2/3 h-[500px] lg:h-auto min-h-[500px] bg-slate-900 rounded-2xl overflow-hidden border border-slate-700 relative shadow-inner">
               <DisasterMap 
                 regions={[{ 
                   ...selectedEvent, 
               ...
               <div className="absolute top-6 right-6 z-[1000] bg-slate-900/80 backdrop-blur-sm p-4 rounded-xl border border-slate-600 shadow-xl pointer-events-none text-right">
                 <div className="text-[10px] text-slate-400 uppercase font-bold tracking-widest mb-1">Simulated Radar</div>
                 <div className={`text-2xl font-black transition-colors duration-500 ${currentRiskLevel === 'red' ? 'text-red-500' : 'text-slate-200'}`}>
                   T - {hoursRemaining}H
                 </div>
               </div>
            </div>
